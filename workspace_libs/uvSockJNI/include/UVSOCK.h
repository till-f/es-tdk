...
/** Information about watch expression, variable or its member
  ...
  * @li #UV_DBG_EVUM_VAR_MEMBERS (uVision writes @a VARINFO array results of enumeraton)
  * @li #UV_DBG_EVUM_VARIABLES  (uVision writes @a VARINFO array results of enumeraton)
  * 
  */
...
/** Memory Read / Write data
  ...
  */
typedef struct amem  {
  xU64             nAddr;   ///< Address to read / write /16 byte unsigned data type
  UINT            nBytes;   ///< Number of bytes read / write 4 byte
  xU64           ErrAddr;   ///< Unused 16 byte
  UINT              nErr;   ///< Unused 4 byte
  xUC8         aBytes[1];   ///< @a nBytes of data read or to be written /1 byte
} AMEM;
...
/** Error response data
  ...
  * #UV_STATUS_SUCCESS or if within a #UVSOCK_ASYNC_MSG.
  *
...
/** UVSOCK Command response / async message data format
  ...
  * If status is not #UV_STATUS_SUCCESS, or it within a #ASYNC_MSG packet, then the union will be of type @a err.
