...
/**
 * The Class ProcessorPropertySheet will be called if the connected business object is a Processor.
 * This checks the Class ProcessorFilter.
 * The property sheet has a text field for the name.
 */
public class ProcessorPropertySheet extends GFPropertySection implements ITabbedPropertyConstants {
	...
	public void refresh() {
		...
		if (pe != null) {
			...
			if (bo == null)
				return;
			processor =  (Processor) bo;
			String nameValue = processor.getName();
    ...
    private void changePropertyValue() {
    	...
    	if (!newValue.equals(processor.getName())) {
    		processor.setName(newValue);
			final PictogramElement pe = getSelectedPictogramElement();
			GraphicsAlgorithm graphicsAlgorithm = pe.getGraphicsAlgorithm().getGraphicsAlgorithmChildren().get(0);
			if (null != graphicsAlgorithm
				...
				text.setValue(processor.getName());
			}
