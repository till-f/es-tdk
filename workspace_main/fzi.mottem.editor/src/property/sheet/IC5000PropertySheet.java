...
/**
 * The Class IC5000PropertySheet will be called if the connected business object is a 
 * IC5000 or IC5000Port. This checks the Class IC5000Filter.
 * The property sheet has text fields for name, runtime inspector class, trace controller class and workspace path.
 * There is also a button to change the workspace path, which is a file with the file extension .xjrf.
 */
public class IC5000PropertySheet extends GFPropertySection implements ITabbedPropertyConstants {

	/** The inspector container. */
	private IC5000 inspectorContainer;
		...
		changeWorkspacePathButton.addSelectionListener(new SelectionListener()
			{
				...
				public void widgetSelected(SelectionEvent e)
				{
					...
					if (pe != null)
					{
						...
						{
							inspectorContainer =  (IC5000) bo;
						}
						else if(bo instanceof IInspector)
						{
							inspectorContainer =  (IC5000) ((IInspector) bo).eContainer();
						}
	...
	public void refresh() {
		...
		if (pe != null) {
			...
			if( inspectorContainer.getPort() != null ){
				if( inspectorContainer.getPort().getTraceControllerClass() != null ){
					traceControllerClass = inspectorContainer.getPort().getTraceControllerClass();
				}
			}
    ...
    private void changeTraceControllerClassPropertyValue() {
    	...
    	if (!newValue.equals(inspectorContainer.getPort().getTraceControllerClass())) {
    		inspectorContainer.getPort().setTraceControllerClass(newValue);
    	}
    }
